...
# Уберу дублирующие значения путем создания словаря из списка
vlans_dict = dict.fromkeys(vlans)
# Из полученного словаря снова сделаю список, отсортирую по порядку и запишу в переменную.
result = sorted(list(vlans_dict))
print(result)